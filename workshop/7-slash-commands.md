...
4. Copilot CLI will prompt you to choose where to share the session. Select **gist** to publish the session as a secret GitHub gist.
5. In just a couple of moments, Copilot will create a gist and display the link.
6. Copy the link text.
7. In a new browser tab, paste the link to explore the gist. Note how the gist highlights the prompts sent, skills and agents used, Copilot's thought process, and even the code and results from locally run commands.
